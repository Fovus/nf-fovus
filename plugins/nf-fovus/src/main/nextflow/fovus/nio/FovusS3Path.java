...
import nextflow.fovus.nio.FovusS3FileSystem;
...
public class FovusS3Path implements Path, TagAwareFile {
	 ...
	 * actual filesystem
	 */
	private FovusS3FileSystem fileSystem;
	 ...
	 *
	 */
	public FovusS3Path(FovusS3FileSystem fileSystem, String path) {
     ...
     */
	public FovusS3Path(FovusS3FileSystem fileSystem, String first,
                   String ... more) {
    ...
    private FovusS3Path(FovusS3FileSystem fileSystem, String bucket,
	...
	public String getKey() {
        ...
        if(FovusS3Path.validateCombinedHash(mutableParts, 32)){
	...
	@Override
	public FovusS3FileSystem getFileSystem() {
	...
	public Path getRoot() {
		if (isAbsolute()) {
			return new FovusS3Path(fileSystem, bucket, ImmutableList.<String> of());
		}
	...
	public Path getFileName() {
		if (!parts.isEmpty()) {
			return new FovusS3Path(fileSystem, null, parts.subList(parts.size() - 1,
					parts.size()));
		}
        else {
	...
	public Path getParent() {
		...
		return new FovusS3Path(fileSystem, bucket,
	...
	public Path getName(int index) {
		return new FovusS3Path(fileSystem, null, parts.subList(index, index + 1));
	}

	@Override
	public Path subpath(int beginIndex, int endIndex) {
		return new FovusS3Path(fileSystem, null, parts.subList(beginIndex, endIndex));
	}
	...
	public boolean startsWith(Path other) {
		...
		if (!(other instanceof FovusS3Path)){
			return false;
		}

        FovusS3Path path = (FovusS3Path) other;
	...
	public boolean startsWith(String path) {
        FovusS3Path other = new FovusS3Path(this.fileSystem, path);
	...
	public boolean endsWith(String other) {
		return this.endsWith(new FovusS3Path(this.fileSystem, other));
	}
	...
	public Path normalize() {
		...
		return new FovusS3Path(fileSystem, bucket, normalize0(parts));
	}
	...
	public Path resolve(Path other) {
		...
		return new FovusS3Path(fileSystem, bucket, concat(parts, s3Path.parts));
	}

	@Override
	public Path resolve(String other) {
		return resolve(new FovusS3Path(this.getFileSystem(), other));
	}
	...
	public Path resolveSibling(Path other) {
		...
		return new FovusS3Path(fileSystem, bucket, concat(
				parts.subList(0, parts.size() - 1), s3Path.parts));
	}

	@Override
	public Path resolveSibling(String other) {
		return resolveSibling(new FovusS3Path(this.getFileSystem(), other));
	}
	...
	public Path relativize(Path other) {
		...
		if (this.equals(other)) {
			return new FovusS3Path(this.getFileSystem(), "");
		}
		...
		return new FovusS3Path(fileSystem, null, resultParts);
	}
	...
	public Iterator<Path> iterator() {
		...
		for (Iterator<String> iterator = parts.iterator(); iterator.hasNext();) {
			...
			builder.add(new FovusS3Path(fileSystem, null, ImmutableList.of(part)));
		}
	...
	public boolean equals(Object o) {
        ...
        FovusS3Path paths = (FovusS3Path) o;
