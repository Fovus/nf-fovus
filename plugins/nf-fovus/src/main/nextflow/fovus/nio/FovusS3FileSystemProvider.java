 ...
 *
 */
public class FovusS3FileSystemProvider extends FileSystemProvider implements FileSystemTransferAware {

    private static final Logger log = LoggerFactory.getLogger(FovusS3FileSystemProvider.class);

    final Map<String, FovusS3FileSystem> fileSystems = new HashMap<>();
    ...
    public DirectoryStream<Path> newDirectoryStream(Path dir, DirectoryStream.Filter<? super Path> filter) throws IOException {
        ...
        Preconditions.checkArgument(dir instanceof FovusS3Path, "path must be an instance of %s", FovusS3Path.class.getName());
        final FovusS3Path s3Path = (FovusS3Path) dir;
...
//        log.trace("options: {}", options.toString());
        ...
        InputStream result;
//        Path file = Files.createTempFile("/tmp", "fovus-s3");
...
//        return null;
    ...
    public boolean canUpload(Path source, Path target) {
        return FileSystems.getDefault().equals(source.getFileSystem()) && target instanceof FovusS3Path;
    }

    @Override
    public boolean canDownload(Path source, Path target) {
        return source instanceof FovusS3Path && FileSystems.getDefault().equals(target.getFileSystem());
    }

    @Override
    public void download(Path remoteFile, Path localDestination, CopyOption... options) throws IOException {
        final FovusS3Path source = (FovusS3Path) remoteFile;
...
//		}
    ...
    public void upload(Path localFile, Path remoteDestination, CopyOption... options) throws IOException {
        final FovusS3Path target = (FovusS3Path) remoteDestination;
...
//		}
    ...
    private FovusS3OutputStream createUploaderOutputStream(FovusS3Path fileToUpload) {
        FovusClient fovusClient = fileToUpload.getFileSystem().getClient();
...
//		final S3ObjectId objectId = fileToUpload.toS3ObjectId();
        FovusS3OutputStream stream = new FovusS3OutputStream(fovusClient, fileToUpload, opts);
        return stream;
    }
...
//                        getClient().deleteObject(s3Path.getBucket(), s3Path.getKey());
    ...
    public void createDirectory(Path dir, FileAttribute<?>... attrs)
        ...
        FovusS3Path s3Path = (FovusS3Path) dir;
        ...
        s3Path.getFileSystem()
                .getClient()
                .uploadEmptyDirectory(keyName, s3Path.getFileJobId());
    }
...
//			.deleteObject(s3Path.getBucket(), s3Path.getKey() + "/");
    ...
    public void copy(Path source, Path target, CopyOption... options)
            throws IOException {
        Preconditions.checkArgument(source instanceof FovusS3Path,
                "source must be an instance of %s", FovusS3Path.class.getName());
        Preconditions.checkArgument(target instanceof FovusS3Path,
                "target must be an instance of %s", FovusS3Path.class.getName());
        ...
        FovusS3Path s3Source = (FovusS3Path) source;
        FovusS3Path s3Target = (FovusS3Path) target;
        /*
        ...
        FovusClient client = s3Source.getFileSystem().getClient();
...
//		final long length = sourceObjMetadata.getContentLength();
        final List<Tag> tags = ((FovusS3Path) target).getTagsList();
        final String contentType = ((FovusS3Path) target).getContentType();
        final String storageClass = ((FovusS3Path) target).getStorageClass();
...
//			s3ObjectSummaryLookup.lookup((FovusS3Path)path);
//			return;
//		}
//
...
//	}
    ...
    public <V extends FileAttributeView> V getFileAttributeView(Path path, Class<V> type, LinkOption... options) {
        ...
        if (type.isAssignableFrom(BasicFileAttributeView.class)) {
            try {
                return (V) new FovusS3FileAttributesView(readAttr0(s3Path));
            } catch (IOException e) {
    ...
    private Optional<FovusS3FileAttributes> readAttr1(FovusS3Path s3Path) throws IOException {
        try {
...
//		final FovusClientFactory factory = new FovusClientFactory();
        ...
        return new FovusS3FileSystem(this, fovusClient, uri, props);
    }
...
//     *
//	 * @param path {@link FovusS3Path}
//	 * @return AccessControlList
//	 * @throws NoSuchFileException if not found the path and any child
//	 */
//	private AccessControlList getAccessControl(FovusS3Path path) throws NoSuchFileException{
