...
public final class FovusS3OutputStream extends OutputStream {


    private static final Logger log = LoggerFactory.getLogger(FovusS3OutputStream.class);
     ...
     */
    private final FovusJobClient fovusJobClient;
...
//    private final S3ObjectId objectId;
     ...
     *
     */
    public FovusS3OutputStream(final FovusJobClient fovusJobClient, Path fovuS3Path, S3MultipartOptions request) {
        this.fovusJobClient = requireNonNull(fovusJobClient);
        this.fovuS3Path = requireNonNull(fovuS3Path);
...
//    }
    ...
    public void close() throws IOException {
        if (closed || !(fovuS3Path instanceof FovusPath)) {
            return;
        }
        ...
        try (FileOutputStream out = new FileOutputStream(tempFile, true)) {
            ...
            String basePath = ((FovusPath) fovuS3Path).getKey();   // or however you map bucket
            String filePath = tempFile.getAbsolutePath();
            String jobId = ((FovusPath) fovuS3Path).getFileJobId(); // If available; null otherwise

            // Run CLI upload
            fovusJobClient.uploadJobFile(filePath, basePath, jobId);
