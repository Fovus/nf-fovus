...
import nextflow.fovus.nio.FovusS3OutputStream;
...
import static nextflow.fovus.nio.util.S3UploadHelper.*;
...
public class S3Client {
	...
	public void setCannedAcl(String acl) {
		if( acl==null)
			return;
		this.cannedAcl = acl.contains("-") ? CannedAccessControlList.valueOf(CaseFormat.LOWER_HYPHEN.to(CaseFormat.UPPER_CAMEL,acl))
                : CannedAccessControlList.valueOf(acl);
...
//	public void multipartCopyObject(FovusS3Path s3Source, FovusS3Path s3Target, Long objectSize, S3MultipartOptions opts, List<Tag> tags, String contentType, String storageClass ) {
//
...
//		ExecutorService executor = FovusS3OutputStream.getOrCreateExecutor(opts.getMaxThreads());
...
//	}
	...
	public void downloadFile(FovusS3Path source, File target) {
	...
	public void downloadDirectory(FovusS3Path source, File targetFile) throws IOException {
		//
		...
		FileVisitor<Path> visitor = new SimpleFileVisitor<Path>() {
			...
			public FileVisitResult visitFile(Path current, BasicFileAttributes attr) {
				...
				String sourceKey = ((FovusS3Path) current).getKey();
	...
	public void uploadFile(File source, FovusS3Path target) {
	...
	public void uploadDirectory(File source, FovusS3Path target) {
